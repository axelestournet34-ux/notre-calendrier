...
export function generateStaticParams() {
  return DEMO_MEMORIES.map((m) => ({ date: m.date }))
}
...
export default async function JourPage({ params }: Props) {
  ...
  const souvenirs = DEMO_MEMORIES.filter((m) => m.date === date)
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-text-muted mt-0.5">
                        par {souvenir.author_name}
                      </p>
                    </div>
                    {souvenir.photos.length > 0 && (
                      <span className="text-xs text-text-muted shrink-0">
                        {souvenir.photos.length} photo{souvenir.photos.length > 1 ? 's' : ''}
                      </span>
                    )}
                  </div>
