...
import { QuelMoisClient } from './quel-mois-client'

export type MoisMemory = {
  id: string
  title: string
  date: string
  photoUrl: string
...
export default async function QuelMoisPage() {
  ...
  ).filter((m): m is MoisMemory => m !== null)

  return (
    <>
      <Header title="Quel mois était-ce ?" subtitle="Retrouvez quand c'était" />
      <div className="px-4 lg:px-6 py-6 max-w-2xl mx-auto w-full">
        <QuelMoisClient memories={memories} />
      </div>
    </>
  )
}
