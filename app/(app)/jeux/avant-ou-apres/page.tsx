...
import { AvantOuApresClient } from './avant-ou-apres-client'

export type AvantMemory = {
  id: string
  title: string
  date: string
  photoUrl: string
...
export default async function AvantOuApresPage() {
  ...
  ).filter((m): m is AvantMemory => m !== null)

  return (
    <>
      <Header title="Avant ou après ?" subtitle="Lequel est le plus ancien ?" />
      <div className="px-4 lg:px-6 py-6 max-w-2xl mx-auto w-full">
        <AvantOuApresClient memories={memories} />
      </div>
    </>
  )
}
